...
function log(args) {
    ...
    if (showAll) {
        ...
        for (const branch of branches) {
            try {
                const sha = (0, refs_1.resolveRevision)(branch, repoRoot);
        ...
        try {
            const sha = (0, refs_1.resolveRevision)(startRevision, repoRoot);
            startShas.push(sha);
        }
        catch (e) {
...
function getCommitDiffs(sha, commit, repoRoot) {
    const diffs = [];
