...
exports.getAllIndexEntries = getAllIndexEntries;
exports.clearIndex = clearIndex;
exports.buildTreeFromIndex = buildTreeFromIndex;
...
function writeIndex(index, repoRoot) {
    const minigitDir = (0, utils_1.getMinigitDir)(repoRoot);
    const indexPath = path.join(minigitDir, 'index');
...
function parseIndexEntry(data, offset) {
...
function addToIndex(filePath, sha, mode, stats, repoRoot) {
...
function removeFromIndex(filePath, repoRoot) {
    const index = readIndex(repoRoot);
    const existingIdx = index.entries.findIndex(e => e.path === filePath);
    if (existingIdx >= 0) {
        index.entries.splice(existingIdx, 1);
...
function getIndexEntry(filePath, repoRoot) {
    const index = readIndex(repoRoot);
    return index.entries.find(e => e.path === filePath);
}
function getAllIndexEntries(repoRoot) {
    const index = readIndex(repoRoot);
...
function buildTreeFromIndex(repoRoot) {
    const index = readIndex(repoRoot);
    // Import here to avoid circular dependency
    const { createTree } = require('./objects');
    return buildTree(index.entries, '', repoRoot, createTree);
}
function buildTree(entries, prefix, repoRoot, createTreeFn) {
