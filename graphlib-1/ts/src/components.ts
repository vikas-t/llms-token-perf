...
function hasCycleDirected(graph: NonNullable<ReturnType<typeof getGraph>>): HasCycleResult {
  const WHITE = 0;
  const GRAY = 1;
  const BLACK = 2;
...
function hasCycleUndirected(graph: NonNullable<ReturnType<typeof getGraph>>): HasCycleResult {
...
export function stronglyConnectedComponents(graphId: string): ConnectedComponentsResult {
  const graph = getGraph(graphId);
  if (!graph) {
    return { count: 0, components: [] };
  }
