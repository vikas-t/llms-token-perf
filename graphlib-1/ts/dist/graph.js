...
function initializeFromStorage() {
    if (initialized)
        return;
    initialized = true;
    const data = (0, storage_1.loadData)();
    graphIdCounter = data.graphIdCounter;
    for (const serialized of data.graphs) {
        graphs.set(serialized.id, (0, storage_1.deserializeGraph)(serialized));
    }
}
function persistToStorage() {
    const data = {
        graphIdCounter,
        graphs: Array.from(graphs.values()).map(storage_1.serializeGraph),
    };
    (0, storage_1.saveData)(data);
}
function generateId() {
...
function computeIsConnected(graph) {
...
function computeHasCycle(graph) {
    if (graph.nodes.size === 0)
        return false;
    if (graph.directed) {
...
function subgraph(graphId, nodes) {
    initializeFromStorage();
    const graph = graphs.get(graphId);
    if (!graph) {
        return { id: '', directed: false, weighted: false, node_count: 0, edge_count: 0 };
