# Graph Algorithm Library: Python Implementation

## Task

Implement a graph algorithm library in Python. All 150 tests must pass.
...
**FORBIDDEN:**
- Do NOT install or use any third-party graph libraries (e.g., networkx, igraph)
...
│   ├── types.py         # All type definitions (Graph, Edge, Node types)
│   ├── graph.py         # Graph class, add/remove nodes/edges, basic operations
│   ├── traversal.py     # BFS, DFS implementations
│   ├── paths.py         # shortest_path, all_shortest_paths, has_path (Dijkstra)
│   └── components.py    # connected_components, strongly_connected_components, has_cycle, topological_sort
└── cli.py               # CLI wrapper (under 100 lines)
```

**MANDATORY REQUIREMENTS:**
- `__init__.py` MUST be under 50 lines - it only imports and re-exports from modules
- `cli.py` MUST be under 100 lines - it only handles CLI interface
- Each module (`types.py`, `graph.py`, `traversal.py`, `paths.py`, `components.py`) MUST exist as a separate file
- **MINIMUM 7 FILES REQUIRED** (6 in graphlib/ + cli.py)
...
- Any dataclasses or TypedDicts needed
...
Run tests:
```bash
cd /Users/vikas-t/ws/nq/llms-tokens-perf/graphlib-2
...
- **MUST have modular structure with 7 separate files**
- **`__init__.py` MUST be under 50 lines**
- **`cli.py` MUST be under 100 lines**
