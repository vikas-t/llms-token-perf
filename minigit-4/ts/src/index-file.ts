...
import { IndexEntry, INDEX_SIGNATURE, INDEX_VERSION } from './types';
import { sha1, getIndexPath } from './utils';
...
function readEntry(data: Buffer, offset: number): { entry: IndexEntry; nextOffset: number } {
...
export function writeIndex(repoRoot: string, entries: IndexEntry[]): void {
  const indexPath = getIndexPath(repoRoot);
...
function writeEntry(entry: IndexEntry): Buffer {
...
export function addToIndex(entries: IndexEntry[], newEntry: IndexEntry): IndexEntry[] {
...
export function removeFromIndex(entries: IndexEntry[], name: string): IndexEntry[] {
  return entries.filter((e) => e.name !== name);
}

export function getIndexEntry(entries: IndexEntry[], name: string): IndexEntry | undefined {
...
export function createIndexEntry(
  ...
  sha: string,
  mode: number,
  stat: { ctimeSec: number; ctimeNsec: number; mtimeSec: number; mtimeNsec: number; dev: number; ino: number; uid: number; gid: number; size: number }
): IndexEntry {
  return {
...
export function createIndexEntryFromFile(
  name: string,
  sha: string,
  mode: number,
  fsStat: import('fs').Stats
): IndexEntry {
  return {
