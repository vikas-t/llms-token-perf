import { DiffHunk, DiffResult, DiffStats, DiffOptions, DiffOp } from './types';
 ...
 */
function computeLCS<T>(a: T[], b: T[]): [number, number][] {
  const m = a.length;
  const n = b.length;
...
export function diffLines(
  ...
  const oldLines = splitLines(normalizeLineEndings(oldStr));
  const newLines = splitLines(normalizeLineEndings(newStr));

  // Create comparison versions (for matching purposes)
  const oldCompare = oldLines.map((line) =>
    ignoreWhitespace ? line.trim() : line
  );
  const newCompare = newLines.map((line) =>
    ignoreWhitespace ? line.trim() : line
  );
  ...
  const matchedPairs: [number, number][] = lcsPairs.map(([oi, ni]) => [
    oldIndices[oi],
    newIndices[ni],
  ]);
...
function applyContextFilter(hunks: DiffHunk[], contextLines: number): DiffHunk[] {
  ...
  const include: boolean[] = new Array(hunks.length).fill(false);
 ...
 * Word-by-word diff.
 */
export function diffWords(oldStr: string, newStr: string): DiffHunk[] {
 ...
 */
export function diffChars(oldStr: string, newStr: string): DiffHunk[] {
  ...
  const addChar = (op: DiffOp, char: string) => {
    if (currentOp !== op) {
      flushCurrent();
      currentOp = op;
    }
