...
export function createPatch(
  ...
  const oldContent = normalizeLineEndings(oldStr);
  const newContent = normalizeLineEndings(newStr);

  const oldLines = splitLines(oldContent);
  const newLines = splitLines(newContent);

  // Get diff with full context initially
  const diff = diffLines(oldStr, newStr, { context_lines: Infinity });
...
function applyHunk(
  ...
  for (
    let tryStart = Math.max(0, startLine - fuzzRange);
    tryStart <= Math.min(lines.length - expectedOld.length, startLine + fuzzRange);
    tryStart++
  ) {
    if (tryStart < 0 || tryStart + expectedOld.length > lines.length) continue;
...
export function parsePatch(patch: string): ParsedPatch {
  ...
  if (!oldFile && !newFile && hunks.length === 0) {
    throw new ParseError('Invalid patch format');
  }
