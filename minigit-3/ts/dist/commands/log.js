...
function log(args) {
    ...
    if (all) {
        ...
        for (const branch of branches) {
            const sha = (0, refs_1.resolveRef)(branch, repoRoot);
            if (sha)
                startCommits.push(sha);
        }
    }
    else if (revision) {
        ...
        const head = (0, refs_1.getHeadCommit)(repoRoot);
        if (!head) {
    ...
    while (queue.length > 0 && count < limit) {
        ...
        queue.sort((a, b) => {
            try {
                const objA = (0, objects_1.readObject)(a, repoRoot);
                const objB = (0, objects_1.readObject)(b, repoRoot);
                const commitA = (0, objects_1.parseCommit)(objA.content);
                const commitB = (0, objects_1.parseCommit)(objB.content);
                const tsA = (0, utils_1.parseTimestamp)(commitA.committer.split(' ').slice(-2).join(' ')).timestamp;
                const tsB = (0, utils_1.parseTimestamp)(commitB.committer.split(' ').slice(-2).join(' ')).timestamp;
                return tsB - tsA;
            }
            catch {
                return 0;
...
function printOnelineCommit(sha, commit, showGraph) {
    const short = (0, utils_1.shortSha)(sha);
...
function printFullCommit(sha, commit, showStat, repoRoot) {
...
function printCommitStat(commitSha, parentSha, repoRoot) {
    try {
        const commitObj = (0, objects_1.readObject)(commitSha, repoRoot);
        const parentObj = (0, objects_1.readObject)(parentSha, repoRoot);
        const commit = (0, objects_1.parseCommit)(commitObj.content);
        const parent = (0, objects_1.parseCommit)(parentObj.content);
        ...
        for (const [path, sha] of commitFiles) {
            ...
            if (!parentSha) {
                // New file
                const size = (0, objects_1.readObject)(sha, repoRoot).size;
...
function collectFiles(treeSha, repoRoot, prefix = '') {
