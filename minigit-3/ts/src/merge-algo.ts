...
function diffLines(base: string[], other: string[]): { type: 'equal' | 'delete' | 'insert'; line: string; baseIndex: number; otherIndex: number }[] {
  const dp = lcs(base, other);
  const result: { type: 'equal' | 'delete' | 'insert'; line: string; baseIndex: number; otherIndex: number }[] = [];
...
export function threeWayMerge(
  ...
  while (baseIndex < baseLines.length || oursIndex < oursLines.length || theirsIndex < theirsLines.length) {
    ...
    if (baseIndex < baseLines.length && oursEqual && theirsEqual) {
      ...
      if (baseIndex < baseLines.length) baseIndex++;
      oursIndex++;
      theirsIndex++;
      ...
      const oursConflict: string[] = [];
      const theirsConflict: string[] = [];
