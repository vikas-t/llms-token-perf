 ...
 */
function computeLCS<T>(arr1: T[], arr2: T[]): [number, number][] {
  const m = arr1.length;
  const n = arr2.length;
...
interface DiffOp {
  ...
  otherStart: number;
  otherEnd: number;  // exclusive
}

/**
 * Compute diff blocks between base and other.
 */
function computeDiffBlocks(base: string[], other: string[]): DiffOp[] {
 ...
 */
function mergeEqualBlocks(blocks: DiffOp[]): DiffOp[] {
...
export function merge3(
  ...
  const base = splitLines(normalizeLineEndings(baseText));
  const ours = splitLines(normalizeLineEndings(oursText));
  const theirs = splitLines(normalizeLineEndings(theirsText));
  ...
  // Build maps from base line to what it becomes in ours/theirs
  // For each base line, determine if it's unchanged, deleted, or part of a change region
  type LineState =
    | { type: "unchanged" }
    | { type: "changed"; replacement: string[] }
    | { type: "deleted" };

  const oursLineState: (LineState | null)[] = new Array(base.length).fill(null);
  const theirsLineState: (LineState | null)[] = new Array(base.length).fill(null);

  // Also track insertions at positions (before base line index)
  const oursInsertions: Map<number, string[]> = new Map();
  const theirsInsertions: Map<number, string[]> = new Map();
