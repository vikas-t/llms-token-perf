 ...
 */
function computeLCS<T>(oldArr: T[], newArr: T[], equals: (a: T, b: T) => boolean): [number, number][] {
...
export function diffLines(
  ...
  let oldLineIndices: number[] = oldLines.map((_, i) => i);
  let newLineIndices: number[] = newLines.map((_, i) => i);

  if (ignoreBlankLines) {
    const filteredOld: string[] = [];
    const filteredOldIndices: number[] = [];
    ...
    const filteredNewIndices: number[] = [];
...
function applyContextFilter(hunks: DiffHunk[], contextLines: number): DiffHunk[] {
  ...
  const includeHunk: boolean[] = hunks.map((h) => h.op !== "equal");
