...
function groupIntoUnifiedHunks(
  hunks: { op: string; content: string; old_start?: number; new_start?: number }[],
  oldLines: string[],
  newLines: string[],
  contextLines: number
): UnifiedHunk[] {
  ...
  // Find change regions
  const changes: { startIdx: number; endIdx: number }[] = [];
  let inChange = false;
  let changeStart = 0;
  ...
  const mergedChanges: { startIdx: number; endIdx: number }[] = [];
...
export function parsePatch(patch: string): ParsedPatch {
  ...
  if (hunks.length === 0 && patch.trim() !== "" && !patch.includes("--- ")) {
    throw new ParseError("Invalid patch format: no hunks found");
  }
