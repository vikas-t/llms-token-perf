 ...
 */
function computeLCS(oldArr, newArr, compare) {
 ...
 */
function generateHunksFromLCS(oldArr, newArr, lcs, getContent) {
    const hunks = [];
    let oldIdx = 0;
    let newIdx = 0;
    let lcsIdx = 0;
...
function diffLines(oldText, newText, options = {}) {
    ...
    let oldLineMapping = oldLines.map((_, i) => i);
    let newLineMapping = newLines.map((_, i) => i);
    if (ignoreBlankLines) {
        filteredOldLines = [];
        oldLineMapping = [];
        for (let i = 0; i < oldLines.length; i++) {
            if (!(0, utils_1.isBlankLine)(oldLines[i])) {
        ...
        for (let i = 0; i < newLines.length; i++) {
            if (!(0, utils_1.isBlankLine)(newLines[i])) {
    ...
    // Compute LCS
    const compare = (a, b) => {
        return (0, utils_1.normalizeForComparison)(a, ignoreWhitespace) === (0, utils_1.normalizeForComparison)(b, ignoreWhitespace);
    };
    const lcs = computeLCS(filteredOldLines, filteredNewLines, compare);
    // Generate hunks
    const rawHunks = generateHunksFromLCS(filteredOldLines, filteredNewLines, lcs, (line) => line);
