...
import { MergeResult, Conflict, MergeOptions } from './types';
 ...
 */
function computeLCS(aLines: string[], bLines: string[]): Array<[number, number]> {
...
function mergeChanges(
  ...
  ourLines: string[],
  theirLines: string[],
  oursChanges: ChangeRegion[],
  theirsChanges: ChangeRegion[],
  ...
  while (oursIdx < oursChanges.length || theirsIdx < theirsChanges.length) {
    ...
    const overlap =
      (oursBaseStart < theirsBaseEnd && oursBaseEnd > theirsBaseStart) ||
    ...
    if (!oursIsChange && !theirsIsChange) {
      ...
      for (let i = theirsIdx; i < theirsChanges.length; i++) {
        ...
        if (tr.baseStart >= oursBaseEnd) break;
      ...
      for (let i = oursIdx; i < oursChanges.length; i++) {
        ...
        if (or.baseStart >= theirsBaseEnd) break;
